---
URLpath: /firefly-generation-history
title: Firefly Generation History
published: false
date: 2025-02-15
description: Designed Generation History's shift from browser cache to cloud,
  delivering persistent asset storage and cross-app access for 900M+ Firefly
  generations.
tags: Adobe / Product Design
timePeriod: Jan–Feb 2025
---

<div class="cs-nav-anchor" id="overview" data-label="Overview"></div>

#### Overview

Firefly is Adobe's AI generation platform — images, video, and audio, built into the web and into the Creative Cloud suite. Every generation a user creates lives in their Generation History. Before this project, that history lived nowhere useful: browser cache, on a single machine, gone the moment you cleared your cookies.

I led design on the end-to-end redesign of Generation History — moving it from a browser artifact to a persistent cloud collection, building the full action suite that makes those assets usable (sharing, favoriting, downloading, and more), and extending it into Firefly's flagship apps so users could continue work across tools without starting over.

Generation History has supported **900M+ generations** across image, video, and audio to date. Post-MAX launch, MAU grew 56% and asset reuse grew 61%.

<img src="/generationHistory/Main%20gen%20history.png" alt="Firefly Generation History — cloud-stored asset grid with action controls" />

<div class="cs-nav-anchor" id="the-problem" data-label="The problem"></div>

#### The problem

The original Generation History was a local artifact — stored in browser cache, accessible only on the machine where it was created. Clear your cache or switch devices and your history was gone. There was no way to return to a generation, share it with someone, or bring it into Photoshop without downloading and manually re-importing it.

The gap mattered more than it might seem. Firefly is positioned as a creative starting point — a place to generate the raw material that gets refined in Illustrator, composited in Photoshop, animated in Express. If everything you generated disappeared when you closed the tab, Firefly wasn't a creative starting point. It was a dead end.

The design challenge: move an ephemeral, cache-bound system to a persistent cloud model, then build the interface that makes that shift feel like a feature rather than infrastructure.

<div class="cs-nav-anchor" id="cards-and-actions" data-label="Cards & actions"></div>

#### Cards & actions

The foundation of Generation History is the card — how a generated asset is displayed, identified, and acted on. I designed cards to work across three asset types (image, video, audio) within a shared Spectrum-based system, surfacing the right metadata and the right actions without overloading the UI.

The action set I designed for each asset:

- **Share** — generate a public link for sharing generations outside of Adobe
- **Favorite** — bookmark individual assets for quick access
- **Copy link** — copy a direct URL to a specific generation
- **Add to / Move** — send assets to CC Libraries or other collections
- **Download** — in original format, per asset type
- **Delete** — with confirmation state to prevent accidental loss

<div class="cs-image-row cs-image-row--2">
  <img src="/generationHistory/Actions%20in%20Firefly.png" alt="Action menu on a Generation History card in Firefly" />
  <img src="/generationHistory/Sharing%20a%20public%20link.png" alt="Sharing a public link from a Generation History asset" />
</div>

Designing for three asset types meant thinking through which actions applied to which contexts. Video and audio assets have different download expectations than images. Sharing behavior differs when the asset is a 30-second video versus a still. I worked through action availability states for each type and made sure the UI communicated scope clearly — rather than showing actions that silently fail.

Error states were a meaningful part of the surface: failed cloud loads, assets that couldn't be retrieved, share links that had expired. Each needed a design response that didn't leave users guessing.

<div class="cs-nav-anchor" id="cross-app" data-label="Cross-app"></div>

#### Generation History in flagship apps

The second phase extended Generation History beyond Firefly.com — into the flagship Adobe applications users do their actual work in. I designed a new Generation History tab for apps including Photoshop, giving users direct access to their cloud-stored assets without leaving the tool.

<img src="/generationHistory/Add%20from%20gen%20history%20within%20PS%20desktop.png" alt="Generation History panel open within Photoshop desktop" />

The intent was to eliminate the import step entirely. A user could generate a background in Firefly, switch to Photoshop, and place it directly from their history — no download, no manual import, no context switch. The card system designed for the web had to adapt to a constrained panel width and a different workflow intent: in the web, users browse and manage; in Photoshop, they're looking to place and continue.

<img src="/generationHistory/Add%20from%20gen%20history%20PS.png" alt="Adding an asset from Generation History into a Photoshop document" />

<div class="cs-nav-anchor" id="constraints" data-label="Constraints & tradeoffs"></div>

#### Constraints & tradeoffs

This project had two fixed external deadlines: **Adobe MAX London** and **Adobe MAX Los Angeles**. Engineering bandwidth was limited in ways that mattered to design — specifically, scope restrictions on how much the card component could change. Spectrum components have real flexibility, but moving fast on a shared system means some visual customization gets deferred.

I spent time early mapping which design decisions were load-bearing (asset type differentiation, action availability states, error handling) versus which were cosmetic (card proportions, thumbnail treatment). Prioritizing the former kept the engineering path clear while preserving the decisions that affected usability.

The other constraint: designing for image, video, and audio simultaneously without creating three divergent systems. The Spectrum foundation helped — shared components meant shared interaction patterns, and the differentiation between asset types lived in metadata and actions rather than in card architecture.

<div class="cs-nav-anchor" id="impact" data-label="Impact"></div>

#### Impact

<div class="cs-stats">
  <div class="cs-stats__item cs-stats__item--blue">
    <span class="cs-stats__number">900M+</span>
    <span class="cs-stats__label">Generations stored across image, video, and audio</span>
  </div>
  <div class="cs-stats__item">
    <span class="cs-stats__number">56%</span>
    <span class="cs-stats__label">MAU growth post-MAX — 1.89M → 2.37M</span>
  </div>
  <div class="cs-stats__item">
    <span class="cs-stats__number">61%</span>
    <span class="cs-stats__label">Generation asset reuse MAU growth — 759K → 1.19M</span>
  </div>
  <div class="cs-stats__item">
    <span class="cs-stats__number">13.5% → 16.1%</span>
    <span class="cs-stats__label">Reuse penetration as a share of Firefly MAU</span>
  </div>
</div>

The reuse number is the one that matters. MAU tells you people showed up; reuse tells you they did something with what they found. The 61% growth in generation asset reuse — assets pulled from Generation History into another tool or workflow — is the signal that the cross-app integration landed as intended.

<div class="cs-nav-anchor" id="what-i-learned" data-label="What I learned"></div>

#### What I Learned

The real product shift wasn't storage — it was intent. Moving Generation History to the cloud was the enabling infrastructure, but the growth in reuse only happened because the assets were surfaced where work actually happens. Embedding Generation History in Photoshop was not an incremental improvement on the web experience. It was a different product proposition: your Firefly outputs as a resource within your existing Creative Cloud workflow.

Designing under a shared Spectrum system with constrained engineering time forced prioritization that was genuinely useful. When you can't change everything, you have to decide what the design actually depends on. That pressure clarified which decisions were about the experience and which were about appearance. The card system is better for it.

Designing for three asset types at once is harder than it looks, and easier than doing it three times separately. Starting from a unified model — same card, same action patterns, differentiation only where truly needed — produced a system that's extensible in ways a type-by-type approach wouldn't have been.
